...
module.exports = {
  ...
  async emailSubscribers(meetingId, isNew) {
    const meeting = await strapi.services.meeting.findOne({ id: meetingId }, ['plans', 'committee']);
  ...
  async emailNewMeetings(from) {
    from = from || new Date();
    const meetings = await strapi.services.meeting.find({ createdAt_gt: from, date_gt: from }, ['plans']);
